...
export default function CommunityPosts({ posts, community }) {
    ...
    const displayPosts = () => {
        if (community) {
            return posts.filter(post => post.community_id === community.id).map(post => <PostCard title={post.title} content={post.content} username={post.user.username} time={post.created_at}  />)
        }
    }
